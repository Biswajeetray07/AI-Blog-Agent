...
# -----------------------------
from bwa_backend import app, OUTPUT_DIR
...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """
    Stream graph progress. Yields (kind, payload) tuples.
    Fixed: no longer double-invokes the graph after streaming.
    """
    final_state: Dict[str, Any] = {}

    # Attempt streaming with "updates" mode
    try:
        for step in graph_app.stream(inputs, stream_mode="updates"):
            yield ("updates", step)
            # Accumulate state from stream steps
            if isinstance(step, dict):
                for node_name, node_output in step.items():
                    if isinstance(node_output, dict):
                        final_state.update(node_output)
        # Stream completed — yield the accumulated state as final
        if final_state:
            yield ("final", {**inputs, **final_state})
        return
    except Exception:
        pass

    # Fallback: attempt streaming with "values" mode
    try:
        last_value = None
    ...
    # Last resort: simple invoke (no streaming)
...
def list_past_blogs() -> List[Path]:
    """
    Returns .md files in the output/ directory, newest first.
    """
...
with st.sidebar:
    ...
    if not past_files:
        st.caption("No saved blogs found yet. Generate one above!")
